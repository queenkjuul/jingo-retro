...
var gitMech = {
  ...
  log: function (path, version, howMany, callback) {
    ...
    gitSpawn(
      [
        ...
        '--reverse',
        '--no-notes',
        '--pretty=format:%h%n%H%n%an%n%ae%n%aD%n%ar%n%at%n%s',
        version,
        '--',
        docSubdir + path,
      ],
  ...
  mv: function (path, newPath, message, author, callback) {
    gitSpawn(
      ['mv', '-f', docSubdir + path, docSubdir + newPath],
      function (err) {
        if (err) {
          console.log(err)
          callback(err)
  ...
  lastMessage: function (path, revision, callback) {
    gitSpawn(
      ['log', '-1', revision, '--no-notes', '--pretty=format:%s', '--', docSubdir + path],
      function (err, data) {
  ...
  ls: function (pattern, callback) {
    gitExec(
      ['ls-tree', '--name-only', '-z', '-r', 'HEAD', '--', docSubdir + pattern],
      function (err, data) {
        if (err) {
          data = ''
        }
  ...
  findFile: function (file, callback) {
    gitExec(['ls-files', '-z'], function (err, data) {
      if (err) {
        data = ''
      }
