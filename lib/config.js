  ...
  return {
    ...
    defaults: Object.freeze({
      application: {
        ...
        skipGitCheck: false,
        loggingMode: 1,
        pedanticMarkdown: false, // seems to break things with newest Marked parser
        gfmBreaks: true,
        staticWhitelist: '/\\.png$/i, /\\.jpg$/i, /\\.gif$/i',
        proxyPath: '',
        allowHtml: true, // output is sanitized, so this is safer than before
      },
      ...
      features: {
        markitup: false, // no longer an option
        codemirror: true,
        gravatar: true,
      },
    ...
    validate: function () {
      ...
      if (
        config.authentication.google.enabled &&
        (!config.authentication.google.clientId || !config.authentication.google.clientSecret)
      ) {
        error =
          'Invalid or missing authentication credentials for Google (clientId and/or clientSecret).'
        return false
      }

      if (
        config.authentication.github.enabled &&
        (!config.authentication.github.clientId || !config.authentication.github.clientSecret)
      ) {
        error =
          'Invalid or missing authentication credentials for Github (clientId and/or clientSecret).'
        return false
      }
      ...
      if (
        ...
        (!config.authentication.ldap.url ||
          !config.authentication.ldap.searchBase ||
          !config.authentication.ldap.searchFilter)
      ) {
      ...
      if (config.authentication.alone.enabled) {
        console.warn(
          'Deprecation: Alone authentication is deprecated and should be changed with Local.'
        )
      }
      ...
      if (config.features.markitup) {
        error =
          'Starting with Jingo 1.8, Markitup has been removed as its own Markdown editor.\nPlease use codemirror or use an older version of Jingo'
        return false
      }
