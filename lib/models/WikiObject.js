...
class WikiObject extends Configurable {
  ...
  remove(type = 'Page') {
    return new Promiserr((resolve, reject) => {
      ...
      gitmech.rm(this.filename, type + ' removed (' + this.wikiname + ')', this.author, (err) => {
        if (err) {
          reject(err)
          return
        }
  ...
  fetchMetadata() {
    return new Promiserr((resolve, reject) => {
      ...
      gitmech.log(this.filename, this.revision, (err, metadata) => {
  ...
  fetchHashes(howmany) {
    ...
    return new Promiserr((resolve, reject) => {
      ...
      gitmech.hashes(this.filename, howmany, (err, hashes) => {
  ...
  fetchLastCommitMessage() {
    return new Promiserr((resolve, reject) => {
      ...
      gitmech.lastMessage(this.filename, 'HEAD', (err, message) => {
  ...
  fetchHistory() {
    return new Promiserr((resolve, reject) => {
      ...
      gitmech.log(this.filename, 'HEAD', 30, (err, history) => {
        this.lastCommand = 'log'
  ...
  fetchRevisionsDiff(revisions) {
    return new Promiserr((resolve, reject) => {
      ...
      gitmech.diff(this.filename, revisions, (err, diff) => {
        if (err) {
          this.error = err
        }
  ...
  revert() {
    return new Promiserr((resolve, reject) => {
      ...
      gitmech.revert(this.filename, this.revision, this.author, (err, data) => {
        if (err) {
          this.error = err
          reject(err)
          return
